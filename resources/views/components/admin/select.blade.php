...
<div x-data="{ open: false, val: @js($selKey), lbl: @js($selLabel) }" class="relative" @click.outside="open = false">
    ...
    <button type="button" @click="open = !open"
        ...
        <span x-text="lbl || @js($placeholder)"></span>
    ...
    <div x-show="open" x-cloak
        ...
        <div class="max-h-60 overflow-y-auto custom-scrollbar p-1">
            @foreach($options as $value => $label)
                <button type="button"
                    @click="val = @js($value); lbl = @js($label); open = false; $dispatch('select-change', { value: @js($value), name: @js($name) })"
                    ...
                    <svg x-show="val === @js($value)" x-cloak class="h-4 w-4 shrink-0 fill-current text-brand-500" viewBox="0 0 24 24" fill="none">
