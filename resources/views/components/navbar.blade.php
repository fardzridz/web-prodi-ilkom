<header class="sticky top-0 z-20 bg-line text-ink transition-all duration-300 ease-out will-change-transform">
    <div class="mx-4 px-4 h-20 flex items-center justify-between gap-3 sm:mx-8 sm:px-8 sm:h-24 sm:gap-6 lg:mx-16 lg:px-20 lg:h-28">
        <a href="{{ route('home') }}" class="anim-fade-down anim-delay-1 shrink-0">
            @if($site?->logo)
                <img src="{{ asset('storage/'.$site->logo) }}" alt="Logo {{ $site?->site_name ?: 'Ilmu Komputer' }}" class="h-36 w-auto max-w-[200px] object-contain sm:h-44 sm:max-w-[260px] lg:h-60 lg:max-w-[320px]" loading="eager" decoding="async" fetchpriority="high" width="320" height="68">
            @else
                <picture>
                    <source srcset="{{ asset('assets/images/logo/logo-motion.webp') }}" type="image/webp">
                    <img src="{{ asset('assets/images/logo/logo-motion.gif') }}" alt="Logo {{ $site?->site_name ?: 'Ilmu Komputer' }}" class="h-36 w-auto max-w-[200px] object-contain sm:h-44 sm:max-w-[260px] lg:h-60 lg:max-w-[320px]" loading="eager" decoding="async" fetchpriority="high" width="320" height="68">
                </picture>
            @endif
        </a>
            ...
            <ul class="flex items-center gap-8 text-base">
                @php
                    $navLinks = [
                        ...
                        ['route' => 'journal', 'label' => 'Jurnal', 'delay' => 7],
        ...
        <div class="flex shrink-0 items-center gap-3">
            ...
            <a href="{{ $site?->registration_url ?: 'https://admisi.uniwara.ac.id' }}" target="_blank" rel="noopener" class="anim-fade-down anim-delay-4 btn btn-primary btn-md h-11 hidden lg:flex">
