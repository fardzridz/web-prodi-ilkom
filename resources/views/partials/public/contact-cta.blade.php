...
<section id="{{ $id ?? 'kontak-section' }}" class="contact-section scholarship-section relative overflow-hidden h-[188px] min-h-0 my-16 text-white bg-[rgba(150,26,38,0.92)] max-[1024px]:h-[220px] max-[560px]:h-[256px]">
    ...
    <div class="container scholarship-content contact-content w-[min(100%_-_48px,var(--container))] mx-auto max-[560px]:w-[min(100%_-_32px,var(--container))] relative z-[2] grid h-full min-h-0 grid-cols-[minmax(0,1fr)_auto] gap-[clamp(22px,4vw,56px)] items-center py-0 max-[1024px]:grid-cols-1 max-[1024px]:content-center max-[1024px]:gap-5">
        ...
        <div class="contact-actions flex flex-nowrap items-center gap-3 justify-end max-[1024px]:justify-start max-[560px]:w-full max-[560px]:flex-wrap max-[560px]:gap-2.5">
            <a class="contact-button contact-button-primary inline-flex h-11 items-center justify-center px-6 text-blue-dark text-[13px] font-bold leading-none tracking-[0.03em] uppercase bg-yellow transition-[color,background-color,border-color,transform] duration-[180ms] ease-[ease] hover:-translate-y-px hover:bg-[#ffd04f] focus-visible:-translate-y-px focus-visible:bg-[#ffd04f] max-[560px]:w-full" href="{{ $primaryHref }}">{{ $primaryLabel }}</a>
            <a class="contact-button contact-button-secondary inline-flex h-11 items-center justify-center px-6 text-yellow text-[13px] font-bold leading-none tracking-[0.03em] uppercase border-2 border-yellow bg-transparent transition-[color,background-color,border-color,transform] duration-[180ms] ease-[ease] hover:-translate-y-px hover:border-yellow hover:bg-yellow hover:text-blue-dark focus-visible:-translate-y-px focus-visible:border-yellow focus-visible:bg-yellow focus-visible:text-blue-dark max-[560px]:w-full" href="{{ $secondaryHref }}">{{ $secondaryLabel }}</a>
        </div>
    </div>
</section>
