        ...
        <div class="container w-[min(100%_-_48px,var(--container))] mx-auto max-[560px]:w-[min(100%_-_32px,var(--container))]">
            ...
            <div id="kegiatan-grid" class="visit-grid activities-page-grid grid grid-cols-3 gap-[34px] max-[1024px]:grid-cols-1 mt-9">
                @foreach ($activities as $activity)
                    <article class="visit-card activities-page-card h-full load-more-item{{ $loop->iteration > 6 ? ' is-hidden' : '' }} [&_h3]:mt-6 [&_h3]:mb-2 [&_h3]:line-clamp-2 [&_h3]:min-h-[2.1em] [&_h3]:overflow-hidden [&_h3]:text-blue-dark [&_h3]:font-display [&_h3]:text-[34px] [&_h3]:font-medium [&_h3]:leading-[1.05] [&_h3]:tracking-[-0.035em] [&_p]:text-grey-2" data-activity-date="{{ $activity['date'] }}" data-activity-location="{{ $activity['location'] }}" data-activity-category="{{ $activity['category'] }}" data-activity-slug="{{ $activity['slug'] }}" data-activity-image="{{ asset($activity['image']) }}">
