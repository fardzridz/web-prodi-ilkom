...
(function () {
    ...
    const updateTop = () => {
        ...
        const isTopbarHidden = topbar ? topbar.classList.contains('-translate-y-full') : true;
        const isHeaderHidden = header.classList.contains('-translate-y-full');
        const effectiveTopbarH = isTopbarHidden ? 0 : topbarH;
        const effectiveHeaderH = isHeaderHidden ? 0 : headerH;
        // Beranda: topbar sticky sampai Sambutan sentuh Header, jadi pakai class bukan y
        // Fallback untuk non-sticky (sebelum JS sticky aktif): tetap hitung visibleTopbar
        hero.style.top = (effectiveHeaderH + effectiveTopbarH) + 'px';
    };
    updateTop();
    window.addEventListener('scroll', updateTop, { passive: true });
    window.addEventListener('resize', updateTop);
    // Observe class changes (topbar/header translate) untuk update hero secara reaktif
    const mo = new MutationObserver(updateTop);
    if (topbar) mo.observe(topbar, { attributes: true, attributeFilter: ['class'] });
...
<section class="fixed left-0 right-0 top-0 z-0 h-[560px] overflow-hidden md:h-[700px] lg:h-[631px] 2xl:h-[846px]" data-hero aria-roledescription="carousel" aria-label="Sorotan program studi">
    ...
    <div class="hero-slide absolute inset-0 z-0 @if($index > 0) opacity-0 pointer-events-none @else opacity-100 @endif transition-opacity duration-700" data-slide role="group" aria-label="Slide {{ $index + 1 }}">
        <img src="{{ $slide['url'] }}" alt="{{ $slide['alt'] }}" class="absolute inset-0 z-0 h-full w-full object-cover" @if($index === 0) loading="eager" fetchpriority="high" decoding="async" width="1920" height="1080" @else loading="lazy" decoding="async" width="1920" height="1080" @endif>
        <div class="absolute inset-0 z-10 bg-gradient-to-r from-badge/85 via-badge/45 to-badge/10"></div>
    </div>
    @endforeach

    {{-- Overlay teks tunggal: satu h1 per halaman, tidak diduplikasi per slide --}}
    <div class="absolute inset-0 z-20 flex items-center mx-4 px-4 sm:mx-8 sm:px-8 lg:mx-16 lg:px-20" data-hero-copy>
        <div class="max-w-2xl text-cream">
            ...
            <a href="{{ $heroCtaUrl }}" class="anim-zoom anim-delay-{{ count($heroLines) + 2 }} btn btn-primary btn-lg">
                <span class="btn-label">{{ $cta_text }}</span>
    ...
    <div class="mx-4 px-4 sm:mx-8 sm:px-8 lg:mx-16 lg:px-20">
        <div class="mb-12 text-center lg:mb-16" data-reveal>
            <h3 class="inline-flex items-center gap-2.5 uppercase tracking-widest text-primary text-base font-semibold anim-fade-up anim-delay-1">
                ...
                Selamat datang di {{ $site?->site_name ?: 'Program Studi Ilmu Komputer' }}
            </h3>
            <h2 class="mt-4 font-display text-3xl font-bold uppercase tracking-wide text-primary sm:text-4xl lg:text-5xl anim-fade-up anim-delay-2">
        ...
        <div class="mx-auto grid max-w-3xl items-center gap-12 md:grid-cols-[minmax(0,auto)_minmax(0,1fr)] md:gap-14 lg:max-w-4xl">
            <div class="justify-self-center">
                @if ($homeSection?->welcome_image)
                    <img src="{{ asset('storage/'.$homeSection->welcome_image) }}" alt="{{ $homeSection?->welcome_title ?: 'Sambutan Program Studi' }}" class="h-auto w-72 max-w-full object-contain drop-shadow-xl sm:w-80 lg:w-96" loading="lazy" decoding="async" width="600" height="600">
                @else
                    <img src="https://placehold.co/600x600/132845/FDC72F?text=FOTO+Kaprodi&font=roboto" alt="Foto Ketua Program Studi" class="h-72 w-72 object-cover sm:h-80 sm:w-80 lg:h-96 lg:w-96" loading="lazy" decoding="async" width="600" height="600">
                @endif
            </div>
...
<section class="relative z-10 bg-primary pt-16 lg:pt-24">
    <div class="mx-4 px-4 pb-16 sm:mx-8 sm:px-8 lg:mx-16 lg:px-20 lg:pb-24">
        <div class="text-center" data-reveal>
            <h2 class="inline-block font-display text-3xl font-bold uppercase tracking-wide text-cream sm:text-4xl lg:text-5xl anim-fade-up anim-delay-1">{{ $advantageSection['heading'] }}</h2>
        ...
        <div class="mx-auto grid max-w-6xl gap-x-16 gap-y-14 sm:grid-cols-2">
            ...
            <article class="grid grid-cols-2 items-start gap-x-5 gap-y-4 sm:grid-cols-[minmax(0,140px)_auto_minmax(0,1fr)] sm:gap-x-6 anim-fade-up anim-delay-{{ ($loop->index % 3) + 1 }}" data-reveal>
                <div class="relative aspect-square w-full overflow-hidden rounded-lg border border-white/10">
                    <img src="{{ $item['image_url'] }}" @if(!empty($item['image_srcset'])) srcset="{{ $item['image_srcset'] }}" sizes="400px" @endif alt="{{ $item['image_alt'] }}" class="absolute inset-0 h-full w-full object-cover" loading="lazy" decoding="async" width="400" height="400">
                </div>
...
<section class="relative z-10 bg-line py-16 lg:py-24">
    <div class="mx-auto max-w-6xl px-4 sm:px-8 lg:px-16 xl:px-0">
        <div class="mx-auto grid max-w-6xl items-center gap-12 md:grid-cols-[minmax(0,auto)_minmax(0,1fr)] md:gap-20">
            <div class="justify-self-center">
                @if($programProfile?->description_image)
                    <img src="{{ asset('storage/'.$programProfile->description_image) }}" alt="Profil {{ $site?->site_name ?: 'Program Studi Ilmu Komputer' }}" class="aspect-square w-full max-w-sm object-cover sm:max-w-md lg:max-w-lg" loading="lazy" decoding="async" width="512" height="512">
                @else
                    <picture>
                        <source srcset="{{ asset('assets/images/image.webp') }}" type="image/webp">
                        <img src="{{ asset('assets/images/image.png') }}" alt="Profil {{ $site?->site_name ?: 'Program Studi Ilmu Komputer' }}" class="aspect-square w-full max-w-sm object-cover sm:max-w-md lg:max-w-lg" loading="lazy" decoding="async" width="512" height="512">
                    </picture>
                @endif
            </div> 
            <div data-reveal class="anim-fade-up anim-delay-1">
                <h3 class="inline-flex items-center gap-2.5 uppercase tracking-widest text-primary text-base font-semibold anim-fade-up anim-delay-1">
                    ...
                    Tentang {{ $site?->site_name ?: 'Program Studi Ilmu Komputer' }}
                </h3>
        ...
        <div class="anim-fade-up anim-delay-1" data-reveal>
            <div class="mb-12 flex flex-wrap items-end justify-between gap-6 lg:mb-16">
                <div class="text-left">
                    ...
                    <h2 class="mt-4 font-display text-3xl font-bold uppercase tracking-wide text-primary sm:text-4xl lg:text-5xl anim-fade-up anim-delay-2">Kegiatan</h2>
        ...
        <div class="anim-fade-up anim-delay-1" data-reveal>
            <div class="mb-12 flex flex-wrap items-end justify-between gap-6 lg:mb-16">
                <div class="text-left">
                    ...
                    <h2 class="mt-4 font-display text-3xl font-bold uppercase tracking-wide text-cream sm:text-4xl lg:text-5xl anim-fade-up anim-delay-2">Alumni</h2>
